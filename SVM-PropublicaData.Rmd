---
title: "SVM with ProPublica COMPAS Data"
...
No matter where the line is drawn, it is impossible to always correctly classify an individual as re-offender or not. So false positives and false negatives will be inevitable. 
...
data with a ratio of 75-to-25 for training-to-test data.  
...
There will be about
5,500 data points for training and 1,800 for testing. 
...
Next, plot the decision boundary (line) across the data. We need to extract 
the parameters from the SVM model. This is cumbersome. 
...
To reflect the terminology of the debate between ProPublica and Northpointe, we should think in terms of classification and prediction errors. False negative classification (FNC) and false positive classification (FPC) rates (which are used in the ProPublica article about COMPAS) are the 
complement of sensitivity and specificity. 
...
used for COMPAS and apply it to our SVM linear model.  

First, we need to divide 
our test sets and predictions by race.
...
Next, we calculate the error rates 
for predictions and classifications using 
...
if there are race-specific patterns.
