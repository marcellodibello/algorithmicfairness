...
`r tufte::margin_note("We will focus on applications in the criminal justice context. Here, predictive algorithms can be deployed to make decisions about police allocation, preventative detention or bail (pre-trial) or the severity of punishment (post-trial).")` 
Predictive algorithms assign *risk scores* to individuals in order to predict a behavior 
or condition: committing a crime, being a crime victim, defaulting on a loan, a medical condition, etc. They rely on machine learning algorithms (i.e. meta-algorithms) 
to identify correlations between risk factors and the outcome of interest. 
...
is to predict failure to appear in court and new criminal arrest. 
...
Northpointe developed COMPAS (short for Correctional Offender Management Profiling for Alternative Sanctions) as a case management support 
tool for criminal justice practitioners. It was designed for pretrial, jail, probation, prison and 
parole decisions. It calculates two main risks scores, one for *general recidivism* and another for *violent recidivism*.
Scores are meant to convey the probability that someone would re-offend 
within a certain period of time (usually two years). Risk scores range 
from 1 (lowest) to 10 (highest). 

How are risk scores calculated? To illustrate, the violent recidivism risk score depends on the following risk factors: history of noncompliance; education; current age; age-at-first-arrest; history of violence. Each factor 
...
>
> Violent Recidivism Risk Score = (agex x w)+(age-at-first-arrestx x w)+(history of violence x
w) + (vocation education x w) + (history of noncompliance x w) (p. 29).^[Northpointe (2015), [Practitioner’s Guide to COMPAS Core](https://s3.documentcloud.org/documents/2840784/Practitioner-s-Guide-to-COMPAS-Core.pdf)]

# Scores distribution

```{r, echo=FALSE, fig.margin = FALSE, fig.cap="COMPAS risk score distribution. The distribution is different across racial groups. On average, black people have higher risk score than white people. ", out.width = '100%'}
...
ProPublica singled out the group of non re-offenders and compared the percentage of whites in that group mis-classified 
as high risk to the percentage of blacks in the same group also mis-classified as high risk. It also singled out 
the group of re-offenders and compared the percentage of whites in that group mis-classified as low risk to the 
percentage of blacks in the same group also mis-classified as low risk.
...
in this group who are not re-offenders to the percentage of blacks in the same group who are not re-offenders. It also singled out the group of those labeled 'low risk' by COMPAS and compared the percentage of whites in this group who are re-offenders to the percentage of blacks in the same group who are re-offenders. `r tufte::margin_note("As is customary, keep in mind this analysis uses rearrest as a proxy for actual recidivism since we have no way to detect actual criminal activity besides its proxy (such as arrests, convicitions, etc.)")`
...
- *Classification parity*:  Equal classification error rates---that is, that is, equal FPC and FNC---across groups

- *Predictive parity:* Equal prediction error rates---that is, equal FPP and FNP---across  groups

COMPAS satisfies predictive parity fairness, not classification parity.`r tufte::margin_note("We will see later in the semester that no algorithms can satisfies both conceptions of algorithmic fairness under realistic conditions")` The natural question to ask at this point is, *which of the two conceptions (if any) of fairness should we pick?*

# **Three approaches**

Suppose we believe that fairness (justice) requires to treat the like alike, that is, to treat \textit{similarly situated} individuals the same. The question is how we identify similarly situated individuals when we make predictions about criminal behavior. 
...
>
The demand for equal algorithmic treatment for same-outcome groups amounts to a judgment that outcomes are the appropriate basis for prediction. And that judgment is nonsensical (p.\ 2275).^[ Mayson, `Bias In, Bias Out,' *Yale Law Journal*, 2019, 128: 2218-2300.]
...
re-offenders the same and should treat non-reoffenders the same.^[This anaology is partly based on Hellman, 'Measuring Algorithmic Fairness,' *Virginia Law Review*, forthcoming] 


## Neither classification nor predictive parity

The previous two argument still assume that algorithmic fairness requires equal distribution of errors, but disagree on how errors should be equalized. It is, however, not obvious that errors should be equalized, perhaps it is harm inflicted to people that should be equalized. 
...
>
> what should matter is the absolute cost of using a coercive tactic against a member of a minority group, net of benefit, for all members of that racial group. (pp.\ 1127-28)^[Huq, 'Racial Equity in Algorithmic Criminal Justice,' *Duke Law Journal*, 2019, 68: 1043-1134.]
